...
class MigratePuppetdbApiVersionSetting < ActiveRecord::Migration[4.2]
  def up
    puppetdb_address = Setting.where(category: 'Puppetdb', name: 'puppetdb_address').first.try(:value)
    return unless puppetdb_address

    api_version = URI.parse(puppetdb_address).path.start_with?('/pdb') ? 3 : 1
    setting = Setting.where(category: 'Puppetdb', name: 'puppetdb_api_version').first_or_create
